...
[1.0,1.0,0.4,0.8,1.0,1.0,0.4,0.8,1.0,0.7,0.4,0.8,1.0,1.0,0.4,0.8] @=> float fixedDynamics[];

// curva de paneo
[0.0,0.4,-0.6,-0.8,0.0,0.4,0.6,0.8,0.0,0.4,-0.6,-0.8,0.0,0.4,0.6,0.8] @=> float fixedPan[]; // altere estos valores para obtener diferentes formas de paneo
...
// --hiHat
Noise hhImpulse => ResonZ hhFilter => ADSR hh => Pan2 hhPan => dac;
...
fun void playDrums()
{
  ...
  while(true)
  {
    hhGain * fixedDynamics[i] => hhImpulse.gain; 
    fixedPan[i] => hhPan.pan; // comente y descomente esta línea para escuchar variación en el paneo
    // Std.fabs(fixedPan[i]) => hhPan.pan; // variación que solo genera valores positivos entonces solo panea a la derecha.
    // Std.sgn(fixedPan[i]) => hhPan.pan; // variación que lleva a los extremos los paneos pues computa los valores intermedios a -1.0 o 1.0
...
Machine.add(me.dir() + "/gen_spaciality_001.ck");
